"""Build and train multiscalenet with options for quantization."""
...
from slim_utils.datasets import dataset_factory
from slim_utils.preprocessing import preprocessing_factory
...
def build_model():
  ...
  with g.as_default(), tf.device(
    ...
    labels = tf.placeholder(tf.int32, [None, FLAGS.num_classes], name='labels')
    model = multiscalenet.Multiscalenet(is_training=True,
                                        num_classes=FLAGS.num_classes,
                                        scale_list=[1, 2],
                                        depth_multiplier=FLAGS.depth_multiplier,
                                        crop_size=[FLAGS.image_size, FLAGS.image_size])
    logits, _ = model.build_model(inputs)
